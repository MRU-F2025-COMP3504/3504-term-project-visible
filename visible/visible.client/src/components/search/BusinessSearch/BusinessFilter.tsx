...
//define that BusinessFilter is looking for a setter function
type businessFilterProps = {
  setSearchKeyword: Dispatch<SetStateAction<string>>;
};

//BusinessFilter component implements the form to filter the business list
const BusinessFilter = ({ setSearchKeyword }: businessFilterProps) => {
    ...
    <div className="flex justify-end">
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="max-w-[600px] bg-black!">
          <DialogHeader>
            <DialogTitle>Search Businesses</DialogTitle>
          </DialogHeader>
          {/* Description hidden - used by screen readers */}
          <DialogDescription hidden={true}>
            Provide keyword to filter businesses by attributes.
          </DialogDescription>
          <form
            // Submit Function
            onSubmit={(e) => {
          ...
          >
            <Input
              id="keyword"
              type="text"
              placeholder="Search by keyword, industry, or business name"
              className="mt-4"
              value={keywordForm}
              onChange={(e) => {
                e.preventDefault();
